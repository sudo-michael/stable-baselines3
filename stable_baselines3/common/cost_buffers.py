"""
Naming convention: cost_
Add cost code below current code
"""
...
class CostReplayBuffer(CostBaseBuffer):
    """
    ...
    def __init__(
        ...
        if psutil is not None:
            total_memory_usage: float = (
                self.observations.nbytes + self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes + self.costs.nbytes
            )
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> CostReplayBufferSamples:
        """
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> CostReplayBufferSamples:
...
class CostRolloutBuffer(CostBaseBuffer):
    """
    ...
    def cost_compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        """
        Post-processing step: compute the lambda-return (TD(lambda) estimate)
        and GAE(lambda) advantage.
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            delta = self.costs[step] + self.gamma * next_values * next_non_terminal - self.cost_values[step]
            last_gae_lam = delta + self.gamma * self.gae_lambda * next_non_terminal * last_gae_lam
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[CostRolloutBufferSamples, None, None]:
        assert self.full, ""
