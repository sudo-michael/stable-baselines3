...
class CostEvalCallback(EventCallback):
    """
    Callback for evaluating an agent.
    ...
    def __init__(
        ...
        # Logs will be written in ``evaluations.npz``
        if log_path is not None:
            log_path = os.path.join(log_path, f"evaluations_seed{eval_env.seed()[0]}")
        self.log_path = log_path
        self.evaluations_results: list[list[float]] = []
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if self.log_path is not None:
                ...
                if hasattr(self.model, "lagrange"):
                    self.evaluations_lambda.append(self.model.lagrange.lagrangian_multiplier.item())
                    kwargs = {"lambda": self.evaluations_lambda}
            ...
            if self.verbose >= 1:
                print(
                    f"Eval num_timesteps={self.num_timesteps}, episode_reward={mean_reward:.2f} +/- {std_reward:.2f}, episode_cost={mean_cost:.2f} +/- {std_cost:.2f}"
                )
                print(f"Episode length: {mean_ep_length:.2f} +/- {std_ep_length:.2f}")
            # Add to current Logger
            self.logger.record("eval/mean_reward", float(mean_reward))
            ...
            if hasattr(self.model, "lagrange"):
                self.logger.record("eval/lambda", self.model.lagrange.lagrangian_multiplier.item())
