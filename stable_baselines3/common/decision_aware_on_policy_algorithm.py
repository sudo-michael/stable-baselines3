...
from stable_baselines3.common.decision_aware_base_class import DecisionAwareBaseAlgorithm
...
SelfOnPolicyAlgorithm = TypeVar("SelfOnPolicyAlgorithm", bound="DecisionAwareOnPolicyAlgorithm")


class DecisionAwareOnPolicyAlgorithm(DecisionAwareBaseAlgorithm):
    """
    ...
    policy: DecisionAwareActorCriticPolicy
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            print(f"starting collecting rollout: {self.num_timesteps=}")
            continue_training = self.collect_rollouts(self.env, callback, self.rollout_buffer, n_rollout_steps=self.n_steps)
            print(f"done collecting rollout: {self.num_timesteps=}")
            ...
            print(f'traning at step: {self.num_timesteps}')
            self.train()
    ...
    def _get_torch_save_params(self) -> tuple[list[str], list[str]]:
        state_dicts = ["policy", "policy.optimizer_act", "policy.optimizer_critic"]
